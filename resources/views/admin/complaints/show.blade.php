    ...
    <div class="bg-white p-6 rounded-lg shadow-xl mb-8">
        <div class="grid grid-cols-2 gap-y-4">
            ...
            <div>
                ...
                <h1 class="text-md text-gray-700 whitespace-pre-line">{{ $complaint->title }} </h1>
           ...
           {{-- Tautan untuk melihat foto/media (jika ada) --}}
            @if($complaint->photos && count($complaint->photos) > 0)
                <div class="photo-gallery mt-4">
                    <p class="text-sm font-medium text-gray-700 mb-3">Lampiran Photo</p>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($complaint->photos as $index => $photo)
                            <div class="photo-item border rounded-lg overflow-hidden shadow-sm">
                                <img 
                                    src="{{ route('complaints.photo', $photo['file_id']) }}" 
                                    alt="Photo {{ $index + 1 }}"
                                    class="w-full h-48 object-cover cursor-pointer hover:opacity-90 transition-opacity"
                                    onclick="openModal('{{ route('complaints.photo', $photo['file_id']) }}')"
                                    onerror="this.style.display='none'; document.getElementById('error-{{ $index }}').style.display='block';"
                                >
                                <div id="error-{{ $index }}" class="hidden p-4 text-center text-gray-500 bg-gray-100">
                                ...
                                <div class="p-2 bg-gray-50 text-center">
                                    <p class="text-xs text-gray-600">Photo {{ $index + 1 }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            @else
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-lg font-medium">Tidak Ada Photo</p>
                    <p class="text-sm">Laporan ini tidak memiliki lampiran foto</p>
                </div>
            @endif
